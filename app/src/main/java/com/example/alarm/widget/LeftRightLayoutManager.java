...
/**
 * Created by rick on 15/7/21.
 */
public class LeftRightLayoutManager extends RecyclerView.LayoutManager {
    public static final int HORIZONTAL = OrientationHelper.HORIZONTAL;
    public static final int VERTICAL = OrientationHelper.VERTICAL;
    public static final int INVALID_OFFSET = Integer.MIN_VALUE;
    private static final String TAG = "LeftRightLayoutManager";
     ...
     * helps {@link LeftRightLayoutManager} make those decisions.
     ...
     */
    public LeftRightLayoutManager(Context context) {
     ...
     */
    public LeftRightLayoutManager(Context context, int orientation, boolean reverseLayout) {
        ...
        LinearSmoothScroller linearSmoothScroller =
                new LinearSmoothScroller(recyclerView.getContext()) {
                    ...
                    public PointF computeScrollVectorForPosition(int targetPosition) {
                        return LeftRightLayoutManager.this
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        ensureLayoutState();
        return computeScrollOffset(state, mOrientationHelper,
    ...
    private int computeScrollExtent(RecyclerView.State state) {
        ...
        ensureLayoutState();
        return computeScrollExtent(state, mOrientationHelper,
                getChildClosestToStart(), getChildClosestToEnd(), this,
    ...
    private int computeScrollRange(RecyclerView.State state) {
        ...
        ensureLayoutState();
        return computeScrollRange(state, mOrientationHelper,
                getChildClosestToStart(), getChildClosestToEnd(), this,
                mSmoothScrollbarEnabled);
    }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        View prevView = layoutState.current(recycler);
        ...
        int xdelta = 0;
        int ydelta = 0;
        if (UIUtils.px2Dip(width) > GROUP_WIDTH) {
            // group
            if (odd) {
                // do nothing
            } else {
                // align to right
                xdelta = screenWidth - width;
            }
        } else {
            // node
            int quarter = screenWidth / 4;

            if (odd) {
                xdelta = quarter - width / 2 + UIUtils.dip2Px(10);
            } else {
                // align to right
                xdelta = quarter * 3 - width / 2 - UIUtils.dip2Px(10);
            }
        }
        left += xdelta;
        right += xdelta;

        final int GROUP_OFFSET = 20;
        final int NODE_OFFSET = 60;

        // bad codes here: prevView.getMeasuredWidth() always return 0
        // so have to detect prevView type as below
        boolean prevIsGroup = false;
        if (prevView != null) {
            SoundSpaceRecyclerAdapter.DeviceViewHolder holder = (SoundSpaceRecyclerAdapter.DeviceViewHolder) prevView.getTag();
            if (holder != null && holder.device.isGroup()) {
                prevIsGroup = true;
            }
        }

        if (layoutState.mLayoutDirection > 0) {
            int tt1 = 0;
            int tt2 = 0;
            if (prevView != null) {
                tt1 = UIUtils.px2Dip(prevView.getWidth());
                tt2 = UIUtils.px2Dip(width);
            }
            if (index == 0) {
                // do nothing
            } else if (prevIsGroup && UIUtils.px2Dip(width) < GROUP_WIDTH) {
                ydelta -= UIUtils.dip2Px(GROUP_OFFSET);
            } else if (UIUtils.px2Dip(width) > GROUP_WIDTH) {
                ydelta -= UIUtils.dip2Px(GROUP_OFFSET);
            } else if (UIUtils.px2Dip(width) < GROUP_WIDTH) {
                ydelta -= UIUtils.dip2Px(NODE_OFFSET);
            }
        } else {
            if (index == 0) {
                if (!prevIsGroup && UIUtils.px2Dip(width) < GROUP_WIDTH) {
                    ydelta = UIUtils.dip2Px(NODE_OFFSET);
                } else {
                    ydelta = UIUtils.dip2Px(GROUP_OFFSET);
                }
        ...
        top += ydelta;
        bottom += ydelta;
        result.mConsumed += ydelta;

        layoutDecorated(view, left + params.leftMargin, top + params.topMargin,
                right - params.rightMargin, bottom - params.bottomMargin);
        if (DEBUG) {
            SoundSpaceRecyclerAdapter.DeviceViewHolder holder = (SoundSpaceRecyclerAdapter.DeviceViewHolder) view.getTag();
            MergeViewBean device = holder.device;
            String name = device.getName();

            String prev_name = "";
            if (prevView != null) {
                holder = (SoundSpaceRecyclerAdapter.DeviceViewHolder) prevView.getTag();
                device = holder.device;
                prev_name = device.getName();
            }
            ...
            Log.d(TAG, "laid out child at position " + getPosition(view) + ", with l:"
                            ...
                            + " name: " + name + " prev_name: " + prev_name
            );
            Log.d(TAG, "laid out child2at position " + getPosition(view) + ", with l:"
     ...
     */
    int computeScrollOffset(RecyclerView.State state, OrientationHelper orientation,
     ...
     * @param endChild   View closest to end of the list (bottom or right)
     */
    int computeScrollExtent(RecyclerView.State state, OrientationHelper orientation,
                            View startChild, View endChild, RecyclerView.LayoutManager lm,
     ...
     * @param endChild   View closest to end of the list (bottom or right)
     */
    int computeScrollRange(RecyclerView.State state, OrientationHelper orientation,
                           View startChild, View endChild, RecyclerView.LayoutManager lm,
                           boolean smoothScrollbarEnabled) {
        if (lm.getChildCount() == 0 || state.getItemCount() == 0 || startChild == null ||
                endChild == null) {
    ...
    static class LayoutState {
         ...
         */
        View current(RecyclerView.Recycler recycler) {
            if (mScrapList != null) {
                return nextFromLimitedList();
            }
            try {
                final View view = recycler.getViewForPosition(mCurrentPosition - mItemDirection);
                return view;
            } catch (Exception e) {
                return null;
            }
        ...
        private View nextFromLimitedList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // FIXME if (!mIsPreLayout && viewHolder.isRemoved()) {
                if (!mIsPreLayout && viewHolder.isRecyclable()) {
                    continue;
                }
